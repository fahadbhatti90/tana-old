document.addEventListener('DOMContentLoaded', function (e) {
    ...
    var validationForm = FormValidation.formValidation(
        document.getElementById('dropship_form'),
        {
            fields: {
                vendor: {
                    validators: {
        ...
        var action_method = '';
        var namesfile = '';
        if ($('#action_button').val() == 'Upload in Dropship') {
        ...
        var formData = new FormData($("#dropship_form").get(0));
        $.noConflict();
        $.ajax({
            url: base_url + "/dropship/store",
            type: 'POST',
            data: formData,
            dataType: 'json',
            processData: false,
            ...
            success: function (data) {
                ...
                if (data.error) {
                    ...
                    html += '</div>';
                    $("#vendor_dropship").val(null);
                    $('#form_result').html(html);
                    $("#action_button").attr("disabled", false);
                    document.getElementById("action_button").value = "Upload in Dropship";
    ...
    $('output').on('click', '.del', function () { //to remove from front end and id as well
        ...
        id = id.split("_");
        id = id[1];
        var x = document.getElementById("vendor_dropship").files;
    ...
    document.getElementById('vendor_dropship').addEventListener('change', handleFileSelect, false);
...
function hide(x) {
    ...
    $('#target_div').append(
        $('<input/>').attr('type', "file").attr('name', "vendor_dropship[]").attr('accept', ".csv,.xlsx").attr('id', "vendor_dropship").attr('class', "custom-file-input").attr('multiple', "multiple").attr('required', "required").attr('onchange', "hide(this)")
    );
...
$('#vendor_dropship').change(function () {
    var i = this.files.length;
    document.getElementById("count").value = i;

    document.getElementById("vendor_dropship").style.visibility = "hidden";
    var myClock = document.getElementById('vendor_dropship');
    myClock.style.display = 'none';
    $('#target_div').append(
        $('<input/>').attr('type', "file").attr('name', "vendor_dropship[]").attr('accept', ".csv,.xlsx").attr('id', "vendor_dropship").attr('class', "custom-file-input").attr('multiple', "multiple").attr('required', "required").attr('onchange', "hide(this)").attr('onmouseout', "getvalue(this)")
    );
